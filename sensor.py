    ...
    try:
        ...
        if protocol == socket.IPPROTO_TCP:  # TCP
            ...
            if flags == 2:  # SYN set (only)
                ...
                #s7comm
                if dst_port==102 or src_port== 102:
                    s7_version, _, s7_length = struct.unpack('!BBH', tcp_data[:4])
                    s7_data = tcp_data[:s7_length]
                    #print(s7_data)
                    s7_data = binascii.b2a_hex(s7_data)#编码请求数据包
                    if s7_version == 0x03:
                        s7comm_data = tcp_data[4:]
                        header = struct.unpack('!BBB', s7comm_data[:3])
                        packet_length = header[0]
                        tpdu_type = int(header[1])
                        trailer = s7comm_data[1 + packet_length:]
                        if tpdu_type == 0xe0:#cotp连接
                            log_event((sec,usec,src_ip,src_port,dst_ip,dst_port,PROTO.S7,TRAIL.IP,'0xe0',"COTP Connection Request",s7_data))
                        if tpdu_type == 0xf0:
                            fixed_header = struct.unpack('!BBHHHH', trailer[:10])
                            magic = int(fixed_header[0])
                            # print('%x' % magic)
                            pdu_type = fixed_header[1]
                            reserved = fixed_header[2]
                            request_id = fixed_header[3]
                            param_length = fixed_header[4]
                            data_length = fixed_header[5]
                            parameters = trailer[10:10 + param_length]
                            data = trailer[10 + param_length:10 + param_length + data_length]
                            param = struct.unpack('!B', parameters[:1])[0]
                            # print('%x'%param)
                            sparam_mapping = {
                                            0x04: 'read',
                                            0x05: 'write',
                                            ...
                                            0x29: 'plc_stop',
                                            0xf0: 's7 connection'}
                            if param in sparam_mapping:
                                log_event((sec,usec,src_ip,src_port,dst_ip,dst_port,PROTO.S7,TRAIL.IP,'%x'%param,sparam_mapping[param],s7_data))

                            elif param == 0x00:#次级功能码
                                sub = struct.unpack('!BHBBBB',parameters[:7])
                                sub_group = sub[4]
                                sub_func = sub[5]
                                sub_group &= 0x0f
                                if sub_group == 0x01:
                                    subfunction = {0x01: 'Request diag data(Type 1)',
                                                      0x02: 'VarTab',
                                                      0x0c: 'Erase',
                                                      0x0e: 'Read diag data',
                                                      0x0f: 'Remove diag data',
                                                      0x10: 'Forces',
                                                      0x13: 'Request diag data(Type 2)'
                                                    }
                                    if sub_func in subfunction:
                                        log_event((sec,usec,src_ip,src_port,dst_ip,dst_port,PROTO.S7,TRAIL.IP,'1-''%x'%sub_func,subfunction[sub_func],s7_data))

                                if sub_group == 0x02:
                                    subfunction = {0x01: 'Memory',
                                                      0x04: 'Unsubscribe',
                                                      0x05: 'Memory2'
                                                    }
                                    if sub_func in subfunction:
                                        log_event((sec,usec,src_ip,src_port,dst_ip,dst_port,PROTO.S7,TRAIL.IP,'2-''%x'%sub_func,subfunction[sub_func],s7_data))

                                if sub_group == 0x03:
                                    subfunction = {0x01: 'List blocks',
                                                      0x02: 'List blocks of type',
                                                      0x03: 'Get block info'
                                                    }
                                    if sub_func in subfunction:
                                        log_event((sec,usec,src_ip,src_port,dst_ip,dst_port,PROTO.S7,TRAIL.IP,'3-''%x'%sub_func,subfunction[sub_func],s7_data))
                                if sub_group == 0x04:
                                    ...
                                    if sub_func in subfunction:
                                        log_event((sec,usec,src_ip,src_port,dst_ip,dst_port,PROTO.S7,TRAIL.IP,'4-''%x'%sub_func,subfunction[sub_func],s7_data))

                                if sub_group == 0x05:
                                    log_event((sec,usec,src_ip,src_port,dst_ip,dst_port,PROTO.S7,TRAIL.IP,'5-''%x'%sub_func,"PLC password",s7_data))
                                if sub_group == 0x07:
                                    subfunction = {0x01: 'Read clock',
                                                      0x02: 'Set clock',
                                                      0x03: 'Read clock (following)',
                                                      0x04: 'Set clock'
                                                      }
                                    if sub_func in subfunction:
                                        log_event((sec,usec,src_ip,src_port,dst_ip,dst_port,PROTO.S7,TRAIL.IP,'7-''%x'%sub_func,subfunction[sub_func],s7_data))
